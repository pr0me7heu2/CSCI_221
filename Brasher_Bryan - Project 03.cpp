 ...
 * gained in more carefully examining the code that I was writing.
 *
 * Submitted 2352 CDT*/
...
Fraction operator /(const Fraction& f1, const Fraction& f2)
{
    Fraction recip = f2.reciprocal();
    int den = f1.den * recip.den;
    int num = f1.num * recip.num;
    Fraction result = Fraction(num, den);
    result.reduce();
    return result;
}
...
bool operator == (const Fraction& f1, const Fraction& f2)
{
    Fraction f = f1 - f2;
    return f.num == 0;
    // fraction is 0 if and only if numerator is 0
}
